...
However, I found some other failing test, which was unexpected:
...
The test fails as we were hoping.

#+BEGIN_SRC sh
#+END_SRC

Lucky us, ANTLR supports that behavior out of the box, just by using a different error handling strategy:
