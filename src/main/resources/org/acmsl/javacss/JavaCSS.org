...
- An ANTLR meta-parser, which reads an ANTLR grammar and generates the StringTemplate templates.
- An AST runtime decorator, which lets StringTemplate access the child nodes via getters.
...
reduce that number to certain extent. But it's a lot of work indeed, and our work is not usable for other languages
in the future.
