...
- Maintain a pointer to the current node in the AST.
- For the current selector, find if the current node matches.
- If it matches,
  - annotate the current node in a list.
  - point to the next node.
  - process the next selector, unless there're no selectors remaining.
- If it doesn't match, then point to the next node and retry.
- Finally, check if the node is included in the list of nodes.
